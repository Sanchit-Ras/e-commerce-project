  ...
  return (
    <div className="md:max-w-80 relative p-2 w-full">
      ...
      {+data.discount_percentage>1 && 
        (<p className="inline text-xs text-gray-400 mr-1">{(data.price / (1 - data.discount_percentage / 100)).toFixed(2)}</p>)}
      <p className="text-md md:text-xs font-medium inline">${data.price}</p> 
      {+data.discount_percentage>1 && 
      (<div className="text-red-500 md:text-sm text-xs font-bold rounded-full size-10 pr-2 pl-1 pt-2 absolute -top-1 -right-1 flex justify-center ">{(data.discount_percentage).toFixed(1)}% OFF</div>)}
      <Link to={'/product/'+data.id} className="text-[#938b5d] font-semibold block">View Details</Link>
    </div> 
  )
}
